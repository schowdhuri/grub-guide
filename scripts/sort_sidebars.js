...
function sortSidebar(items, sortDishes = true, sortCategories = false) {
  ...
  return sorted.map(item => {
    if (item.type === 'category' && item.items) {
      return {
        ...item,
        items: Array.isArray(item.items) && item.items.length > 0 && typeof item.items[0] === 'string'
          ? sortDishes ? sortItems([...item.items]) : item.items
...
const sortedCuisineSidebar = sortItems(sidebars.cuisineSidebar.map(country => ({
  ...country,
  items: country.items.map(category => ({
    ...category,
    items: typeof category.items[0] === 'string'
...
const sortedLocationSidebar = sortItems(sidebars.locationSidebar.map(country => ({
  ...country,
  items: sortItems(country.items.map(city => ({
    ...city,
