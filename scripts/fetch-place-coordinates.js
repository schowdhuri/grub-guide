...
async function fetchPlaceDetailsByName(placeName, locationBias = null) {
  return new Promise((resolve, reject) => {
    ...
    let bias = locationBias
      ? `&location=${locationBias.lat},${locationBias.lng}&radius=50000`
      : "";
        ...
        res.on("end", () => {
          try {
            ...
            if (
              ...
              reject(
                new Error(
                  `API error: ${json.status} - ${json.error_message || "Unknown error"}`
                )
              );
            }
          } catch (e) {
  ...
  for (let i = 0; i < csvPlaces.length; i++) {
    ...
    try {
      ...
      if (placeIdData && placeIdData.type === "coordinates") {
        ...
        console.log(
          `📍 [${i + 1}/${csvPlaces.length}] ${place.title}: Using coordinates from URL`
        );
      } else {
        // Use Text Search API with place name
        // Determine location bias based on place name
        const locationBias =
          place.title.toLowerCase().includes("chiang mai") ||
